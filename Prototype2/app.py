...
def enhance_with_unet(pil_image):
    # Convert grayscale image to RGB if it has only 1 channel
    if pil_image.mode != 'RGB':
        pil_image = pil_image.convert('RGB')
        
    # Apply the same transformations as before
    image = test_transform(pil_image).unsqueeze(0).to(device)
    with torch.no_grad():
...
def enhance_with_llflow(pil_image):
    # Convert grayscale image to RGB if it has only 1 channel
    if pil_image.mode != 'RGB':
        pil_image = pil_image.convert('RGB')
...
def upload_image():
    if request.method == "POST":
        ...
        if file:
            ...
            preprocessed_path = os.path.join(PROCESSED_FOLDER, "preprocessed_" + file.filename)
            ...
            # Convert the preprocessed image to a PIL Image for model input
            pil_image = Image.fromarray(preprocessed_image)

            # Process the image using U-Net model
            final_image_unet = enhance_with_unet(pil_image)

            # Process the image using LLFlowNet model
            final_image_llflow = enhance_with_llflow(pil_image)
            ...
            cv2.imwrite(original_path, cv2.cvtColor(np.array(pil_image), cv2.COLOR_RGB2BGR))
